...
- **G2 関数変更（統計的同等性）**：Step 2（バッチ fwd 素朴 GEMM）＋ Step 3（バッチ bwd、同一 perm 再利用、
  dropped 対の gate 勾配 0 化）の段階に適用。drop・加算順序の変化により bit 一致は要求しない。
  判定は統計的同等性の4項目（同一シード・同一データ順・同一スレッド数・同一スレッド配置。
  単体経路 vs バッチ経路 `--moe-batch` の長時間走行比較）：
  ...
  2. 収束単調性：後半 50% の val 相対差推移を確認。単調増加なら dynamics 変化、安定なら過渡応答と記録する
     （許容判断はしない。推移の事実のみ記録）。
  3. drop 率記録（ステップ毎・合計。§1.2 トリガ >5% とは別に G2 記録として残す）。
     加えて renormalize 発火率（drop を含むトークンのうち kept 残存で再正規化した割合）を記録する。
  4. 勾配ノルム分布：平均・分散が ±10% 以内。
...
- 短感度（20 steps、μ=0.1）：drop初盤5–8%→終盤約4–5%でμ=0.01時と同傾向。
  重み感度の確定には長期走行が必要であり、本節では既定0.01を維持する（調整は設計改訂扱い）。

### 10.6 総合（許容判断禁止）

- 4項目のうち最終val差は合格、単調性は安定、勾配平均は合格・分散は境界超過、
  dropは5.80%で<5%基準・<0.5%目標に未達（ただし6.88%→2.52%の改善傾向）。
- よって§5.4の規則に従い **G2通過としない**（§9と同一 rigor）。G3（Step 4）には進まない。
- 原因仮説（断定・許容判断禁止）：H1（dropによる実効勾配差。終盤drop低下とval差0.29%の
  ...
  次手はdrop抑制条件（例：cap感度は設計改訂として別途）または長期soakでの分離であり、
  本書での追加実行はなし。
