...
- **Stage 2実測の位置づけ（未発現の記録）**：学習側実装（`src/gdn2.c`融合、bit同一）は完了したが、壁時間は±1%ノイズ域で中立。これは「効果なし」ではなく「未発現」：S=64KBがL2常駐する学習域では削減が吸収され、design scale／decode域での発現が見込まれる。decode実装後の再測定を必須とする。
